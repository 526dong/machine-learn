...
//	public ModelAndView toAddProgramPage(HttpServletRequest request) {
		...
		if(null != model){
			...
			try {
				...
				if(null != user){
					...
					if (-1 == model.getId().intValue()) {
						...
						if(null != modelsProgramList && !modelsProgramList.isEmpty()){
							for (int i=0;i<modelsProgramList.size();i++){
								...
								if(modelsProgram.getId().intValue() != model.getId().intValue()){
					...
					model.setCreator(user.getLoginName());
					modelsProgramService.save(model);
                  if(model.getStatus()==1) new Thread(()->putOutExchangeService.sendTOModel(model,((User) request.getSession().getAttribute("risk_crm_user")).getLoginName())).start();
