...
public class ImportUtils {
    ...
    private CommonFileValue.DataFile analysisData(List<String> importList, CommonFileValue.DataFile dataFile,
        ...
        if (isContainsSplitCode(columnTitle, splitReg)) {
        ...
        for (int i = 1; i < importList.size(); i++) {
            ...
            if (isContainsSplitCode(column, splitReg)) {
                //csv，txt split
    ...
    public void judgeSplitCodeIsRight(List<String> importList, String splitCode) {
        ...
        if (size < 10) {
            for (String rowVal:importList) {
                ...
                if (!isContainsSplitCode(rowVal, splitCode)) {
                    flag = false;
                    break;
                }
            }
    ...
    public List<ModelsFileInfo> getRealFileInfoList(List<ModelsFileInfo> fileInfoNameList, List<ModelsFileInfo> fileInfoTypeList){
        ...
        if (fileInfoNameList.size() == fileInfoTypeList.size()) {
            for (int i = 0; i < fileInfoNameList.size(); i++) {
                //name file info right order
                ModelsFileInfo fileInfoName = fileInfoNameList.get(i);
                for (int j = 0; j < fileInfoTypeList.size(); j++) {
                    //type file info list
                    ModelsFileInfo fileInfoType = fileInfoTypeList.get(j);
                    if (fileInfoName.getName().equals(fileInfoType.getName())) {
     ...
     * @param bis
     */
    public String getFileCode(BufferedInputStream bis){
        //校验后的codeType
        String codeType = judgeFileEncode(bis);
        if (!"UTF-8".equalsIgnoreCase(codeType) && !"GBK".equalsIgnoreCase(codeType)) {
    ...
    public String getFileSplitCode(Map<String, Object> params){
        ...
        if (splitCode.isEmpty()) {
            throw new RuntimeException("导入文件报错：文件分隔符为空！");
        } else {
     ...
     * @return
     */
    private boolean isContainsSplitCode(String param, String splitReg){
        if (param == null || param.trim().equals("")) {
            return false;
        }
        Pattern pattern = Pattern.compile("\\s*|"+splitReg);
