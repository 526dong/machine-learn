...
public class PutOutExchangeServiceImpl implements PutOutExchangeService {
    ...
    public void saveOutPut(RecivePutOutBean putOutBean,Map<String,Object> data){
        ...
        try{
            ...
            //发现消息到页面
            SendMsg(program,true,sf,putOutBean.getReqId());
        }catch (Exception e){
            SendMsg(program,false,sf,putOutBean.getReqId());
    ...
    private void SendMsg(ModelsProgram program ,boolean flag,String sf,String reqId){
        ...
        try {
            ...
            send.put("iflog",false);
            push.sendMsgToUser(send, Constants.PUSH_CHANNEL_MODEL,(String)MsgPush.comet_map.get(userReqIdMap.get(reqId)));
        }catch (Exception e){
           logger.error("项目获取结果消息发送失败",e);
        }finally {
            userReqIdMap.remove(reqId);
            if(ifend!=null&&ifend){
                release(program.getId());
            }
        }
     ...
     * @param projectId
     */
    private void release(Integer projectId){
        //commet4J信息推送次数标识释放
        sendMsg.remove(projectId);
        //日志监听结束标志
        WatchDirsListener.done.put(String.valueOf(projectId),false);
        //项目请求释放
        userQueueMap.remove(projectId);
    }

    //执行队列中的下一个请求
    private void doNextRequst(Integer programId){
        //执行下一个请求
        Queue<String> query= userQueueMap.get(programId);
        if(query.size()>0) query.poll();
    }
